...
| Google account strategy | **Dedicated agent account** (`simons.gladys.bot@gmail.com`) with full Google OAuth | Single OAuth flow for Gmail API, Drive, and Docs; isolates agent from personal files |

---

## Repository & State Directory Layout

This repo (`~/gladys`) contains project files at the root. OpenClaw state
lives in the `openclaw/` subfolder (`OPENCLAW_STATE_DIR=~/gladys/openclaw`),
keeping a clear separation between your files and OpenClaw-generated files.
...
| `GOOGLE_CLIENT_ID` | Google | OAuth 2.0 client ID for Gmail, Drive, Docs | Phase 3 |
| `GOOGLE_CLIENT_SECRET` | Google | OAuth 2.0 client secret | Phase 3 |
...
6. Set environment: `export OPENCLAW_STATE_DIR=~/gladys/openclaw`
7. Run `openclaw onboard` to generate config in the state dir.
...
1. Configure the provider/model section in `openclaw/openclaw.json`:
   ```json5
   {
     provider: {
...
- [ ] Review what OpenClaw actually wrote to `openclaw/` — update `.gitignore` to match real directories (current `sandboxes/`, `extensions/` entries are guesses).
...
### 3b — Google Integration (Gmail, Drive, Docs)

> See [`.roadmap/email-integration.md`](.roadmap/email-integration.md) for email & Google integration details.

1. Set up GCP project with OAuth 2.0 credentials (Gmail API, Drive API, Docs API enabled).
2. Complete OAuth flow for `simons.gladys.bot@gmail.com` (dedicated agent account).
3. Share target Docs/Drive folders with the agent's Google account.
4. Install and configure the Google skill in OpenClaw (covers Gmail + Drive + Docs).
5. Test: ask Claude to append a note, verify it appears in the doc.
...
| 2 | Failure behavior | 4 | Confirm proposed error handling table above |
| 3 | Gemini integration | Future | Add as comparison/fallback model, not on critical path |
